...
export const useProjectDataStore = defineStore("projectDataStore", {
		...
		getCommentsForFile: (state) => (filePath: string) => {
			return (
				state.comments.filter(
					(comment: CommentDto) => comment.location.filePath === filePath
				) ?? []
			);
		},
	...
	actions: {
		...
		async loadProjectDataAsync(
			...
			this.backendProvider = new StandardBackendProvider(
				serverBaseUrl,
				newProjectId,
				serverAuthToken
			);
		...
		async fetchRepositoryTree(
			...
			try {
				...
				this.fileTreeData = await provider.getRepositoryTree(
					repositoryUrl,
					branch,
					repositoryAuthToken
				);
		...
		async upsertCommentAsync(commentData: CommentDto): Promise<void> {
			...
			try {
				...
				if (commentData.id) {
					const updatedComment = await this.backendProvider.updateComment(
						commentData.id,
						commentData
					);
		...
		async replyToCommentAsync(parentCommentId: string, commentData: CommentDto): Promise<void> {
			...
			try {
				...
				// Online mode
				serverStore.startSyncing();
				const newReply = await this.backendProvider.replyToComment(
					parentCommentId,
					commentData
				);
		...
		fileContainsFileComment(filePath: string): boolean {
			return this.comments.some(
				(comment: CommentDto) =>
					comment.location.filePath === filePath && comment.type === CommentType.File
			);
		},
